...
Create a public S3 bucket named xfusion-public-21329. Ensure that the bucket allows public access to its objects.
Create a private S3 bucket named xfusion-private-25300. Ensure that the bucket does not allow public access.
...
1. Go to **S3 service** in the AWS Console.
2. Click **Create bucket** → name it `xfusion-public-21329`.
3. Leave region as default (or choose your preferred one).
4. **Uncheck** “Block all public access”.
5. Confirm the warning and create the bucket.
6. After creation, go to **Permissions → Bucket policy** and add:
   ```json
   {
     ...
     "Statement":[{
       ...
       "Principal":"*",
       "Action":["s3:GetObject"],
       "Resource":["arn:aws:s3:::xfusion-public-21329/*"]
     }]
   }
   ```
...
1. In S3, click **Create bucket** → name it `xfusion-private-25300`.
2. Keep **Block all public access** checked.
3. Create the bucket. No bucket policy needed since it’s private.
...
3. Table name: `xfusion-S3CopyLogs`.
4. Partition key: `LogID` (String).
5. Leave other settings default and create.
...
4. Name the role `lambda_execution_role`.

---

## ⚡ Step 5: Create Lambda Function
1. Go to **Lambda service → Create function**.
2. Name: `xfusion-copyfunction`.
3. Runtime: **Python 3.9**.
4. Execution role: select `lambda_execution_role`.
5. Upload your code:
   - In **Code tab**, upload `/root/lambda-function.py`.
   - Replace placeholders in the file:
     - `REPLACE-WITH-YOUR-DYNAMODB-TABLE` → `xfusion-S3CopyLogs`
     - `REPLACE-WITH-YOUR-PRIVATE-BUCKET` → `xfusion-private-25300`
6. Deploy the function.

---

## 🔔 Step 6: Add S3 Trigger
1. In the Lambda function page, go to **Configuration → Triggers**.
2. Add trigger → select **S3**.
3. Bucket: `xfusion-public-21329`.
4. Event type: **PUT (ObjectCreated)**.
5. Save.
...
## ✅ Step 8: Verification
- **Check private bucket**: Go to `xfusion-private-25300` → confirm `sample.zip` is present.
- **Check DynamoDB logs**: Go to `xfusion-S3CopyLogs` → open **Items** → confirm a log entry with:
...
Ans:
vi /root/xfusion-priority-stack.yml
